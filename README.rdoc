...
== locations.rake
This is a task that will use the googlemaps geocode API to turn a twitter user's reported location to a latlong number pair. It is not used in the Zoun System.
...
== playlist.rake
This task is called from the command line in the exact same manenr as information.rake (edit the event name in the file as well), but it populates the setlist information from setlist.fm. In the future this is where the generation of rdio playlists will go.
